...
def run_queries(adapter_name: str, adapter, records: list[dict], rounds: int) -> list[dict]:
    ...
    results.append(build_result(adapter_name, "query_ann", latencies_ann, clients=1))
    ...
    results.append(build_result(adapter_name, "query_filtered", latencies_filt, clients=1))
...
def main():
    ...
    for name, adapter in adapters.items():
        results = run_queries(name, adapter, records, args.rounds)
        for r in results:
